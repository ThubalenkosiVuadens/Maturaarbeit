...
boolean collisionAvoidance = false;
boolean backwards = false;
boolean IrActivated = false;
boolean done = false;
boolean printed = false;
int IrCounter;
//Servos
#include <Servo.h>
Servo ServoHorizontal;//zur Seite, 1600 Mitte
Servo ServoVertikal;// rauf runter(kl. Zahl), 1440 Mitte,
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if (collisionAvoidance && !IrActivated)
  {
    if (millis() > UsTriggerTime + 45) //42 zu hoch(erst nächster Output wird gelesen, 50ms (!) delay), teils noch bei 39, daher 38
    {
      us_distance = int(analogRead(A0) * 1.267); //*5,/1023->Volt,*1000, ->mV,/9.8,*2.54->cm
      Serial1.println(us_distance);
      UsTriggerTime = millis();
  ...
  else
  {
    if (backwards == true && done == false)
    {
      ...
      ServoHorizontal.writeMicroseconds(1134 + 213 * IrCounter);
      ServoVertikal.writeMicroseconds(1074 + row * 213);
      //Serial.println("switch");

    }
    backwards = false;
    //Serial.println("not_backwards");
  }}

  /*
    if(done)
    {
    if(millis()>IrTriggerTime+10000)
    {
      done=false;
      }
    }
  */
  if (IrActivated && !done)
  {
    if (millis() > IrTriggerTime + 127)//eigentlich +10 aber loop dauert 2.4 ms(daher 7), Vielfaches von 10(150) davor für Stellzeit Servos
    {
      if (tfmP.getData(tfDist))
      {
        result[IrCounter + 5 * row] = tfDist;
        IrCounter++;
        //Serial.println("read");
        ...
        if (IrCounter == 5)
        {
          ...
          { //IrActivated = false;
            ...
            */
            //Serial.end();
            ServoHorizontal.writeMicroseconds(1560);
            ServoVertikal.writeMicroseconds(1500);
            done = true;

          }
          else {
            ServoVertikal.writeMicroseconds(1074 + row * 213);
            ServoHorizontal.writeMicroseconds(1134);
            //Serial.println("moved");
            IrTriggerTime = millis() + 150;
          }
  ...
  for (axis = 0; axis < 3; axis++) {
    ...
    //PIDs über Schieberegler
    /*
      //Pitch und Roll(E), für PIDs(Variablen) s. EEPROM.ccp l. 160
      int eSchieberegler = ((rcData[AUX1] - 1000) >> 5); //->0-62.5, Conf-Wert entspricht 1/10
      int fSchieberegler = ((rcData[AUX2] - 1000) >> 4); // * 20/640;, letzter Faktor bestimmt range
      conf.pid[PIDLEVEL].D8 = eSchieberegler;
      //conf.pid[PIDPITCH].P8 = conf.pid[PIDROLL].P8 ;
      conf.pid[PIDLEVEL].I8 = fSchieberegler;
      //conf.pid[PIDPITCH].D8 = conf.pid[PIDROLL].D8 ;

      //conf.pid[PIDLEVEL].P8 = 30; conf.pid[PIDLEVEL].I8 = 32; conf.pid[PIDLEVEL].D8 = 0;
    */
    //PIDs auslesen
    /*
      Serial.println(conf.pid[PIDROLL].P8);
      Serial.println(conf.pid[PIDROLL].D8);
      Serial.println(conf.pid[PIDROLL].I8);
      Serial.println(conf.pid[PIDPITCH].P8);
      Serial.println(conf.pid[PIDPITCH].D8);
      Serial.println(conf.pid[PIDPITCH].I8);
      Serial.println(conf.pid[YAW].P8);
      Serial.println(conf.pid[YAW].D8);
      Serial.println(conf.pid[YAW].I8);
      Serial.end();
    */
...
void setup() {
  ...
  */
  ...
  //Serial2.begin(9600);
  /*
  pinMode(21, OUTPUT);//Pins 21-24 als Pluspole
  ...
  */
  ...
  ServoHorizontal.attach(9);//hoch=links, 213=30grad, 1134,1240,1560,1880,2200
  ServoHorizontal.writeMicroseconds(1560);
  ServoVertikal.attach(10);// rauf runter(kl. Zahl), 860,1180, 1500,1820,2140
  ServoVertikal.writeMicroseconds(1500);
...
void loop () {
  ...
  */
  ...
  for (axis = 0; axis < 2; axis++) {
    rc = rcCommand[axis] << 1;
    /*
    if (collisionAvoidance && axis == 1 && backwards) //axis==1: pitch
    {
      rc = rc - 40; //rc rangiert von 404 bis -404(hinten)
    ...
    */
    ...
    ITerm = (errorGyroI[axis] >> 7) * conf.pid[axis].I8 >> 6;                  // 16 bits is ok here 16000/125 = 128 ; 128*250 = 21300
